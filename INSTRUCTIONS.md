...
## Preparing your spring-boot-admin project

Copy the provided `config` and `catalog` directories including the files they contain into your `spring-boot-admin` project. Also copy the `Tiltfile` to the root directory of `spring-boot-admin` project.

You can now follow the instructions below to deploy and debug your project.

## Working on your spring-boot-admin project
...
```
tanzu apps workload create spring-boot-admin -f config/workload.yaml \
  --local-path . \
...
```
tanzu apps workload get spring-boot-admin
```
